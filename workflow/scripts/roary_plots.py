...
if __name__ == "__main__":
    ...
    roary.drop(
        list(roary.columns[:options.skipped_columns-1]), axis=1, inplace=True)
    ...
    with sns.axes_style('whitegrid'):
        ...
        try:
            ax = plt.subplot2grid(
                (1, 40), (0, 0), colspan=10, facecolor='white')
        ...
        if options.labels:
            ...
            with plt.rc_context({'font.size': fsize}):
                Phylo.draw(t, axes=ax,
                           ...
                           xticks=([],), yticks=([],),
                           ylabel=('',), xlabel=('',),
                           xlim=(-mdist*0.1, mdist+mdist*0.45 -
                                 mdist*roary.shape[1]*0.001),
                           axis=('off',),
    ...
    a = plt.pie([core, softcore, shell, cloud],
                ...
                colors=[(0, 0, 1, float(x)/total)
                        for x in (core, softcore, shell, cloud)],
                autopct=my_autopct)
